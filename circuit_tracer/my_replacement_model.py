...
class ReplacementModel(HookedTransformer):
    ...
    def from_self_pretrained_and_transcoders(
        ...
        **kwargs,
        ...
        # model_name = "meta-llama/Llama-3.2-1B" # 'gpt2-small' # "meta-llama/Llama-3.2-1B"
        ...
        if 'ds' in name:
            ...
            model = super().from_pretrained(backbone_model,hf_model=hf_model, tokenizer=tok, dtype = torch.bfloat16, cache_dir = model_path, device=device)
        else:
            model = super().from_pretrained(model_name, dtype = torch.bfloat16, cache_dir = model_path, device=device)

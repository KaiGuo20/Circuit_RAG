...
def compute_partial_influences(edge_matrix, logit_p, row_to_node_index, max_iter=128, device=None):
    ...
    for _ in range(max_iter):
        ...
        print(f"Warning: Failed to converge after {max_iter} iterations, returning partial result")
...
#         def _hook_fn(grads: torch.Tensor, hook: HookPoint) -> None:
#             # Compute einsum on GPU, then move to CPU and write to CPU buffer
#             out = einsum(
#                 grads.to(output_vecs.dtype)[read_index],
#                 output_vecs,
#                 "batch position d_model, position d_model -> position batch",
#             )
#             proxy._batch_buffer[write_index] = out.detach().cpu()
...
# def compute_partial_influences(edge_matrix, logit_p, row_to_node_index, max_iter=128, device=None):
#     # Force computation on CPU to avoid allocating a huge matrix on GPU
#     device = device or torch.device("cpu")

#     # Ensure inputs are also on CPU
#     edge_matrix = edge_matrix.to(device)
#     logit_p = logit_p.to(device)
#     row_to_node_index = row_to_node_index.to(device)
